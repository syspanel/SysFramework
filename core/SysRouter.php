 ...
 public static function resolve($requestMethod, $requestUri, $dependencies = [])
{
    ...
    // Ajuste de método via POST (_method)
    if ($requestMethod === 'POST' && isset($_POST['_method'])) {
        $requestMethod = strtoupper($_POST['_method']);
    }

    // =========================
    // Normaliza URI (Docker + Nginx)
    // =========================
    $requestUri = $_SERVER['REQUEST_URI'] ?? '/';

    // Remove /index.php no início, se existir
    ...
    // DEBUG: log para verificar path recebido
    error_log("DEBUG SysRouter: Request Path = {$requestPath}, Method = {$requestMethod}");
        ...
        if ($route['method'] === $requestMethod) {
            ...
            if ($params !== false) {
                try {
                    ...
                    self::logError($e->getMessage(), $requestPath);
